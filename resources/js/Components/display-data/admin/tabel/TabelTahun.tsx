...
export default function TabelTahun({
    ...
    return (
        <table className="text-sm w-full">
            <thead>
                <tr>
                    <th className="border-b text-left w-4 py-1 px-2">{inputListTahun.length == 0 && <input type="checkbox" className="w-3 h-3 text-blue-500 bg-gray-100 border-gray-300 rounded focus:ring-transparent" checked={selectAllTableRows.tahun} onChange={() => handleSelectAllChange(selectAllTableRows, dataTahun, setSelectedRows, setSelectAllTableRows, 'tahun')} />}</th>
            ...
            <tbody>
                {dataTahun.map((item, index) => (
                    <tr key={item.id} className={`${selectedRows.tabelTahunRows.some((row) => row.id === item.id) ? 'bg-blue-100' : 'bg-white'} border-b`}>
                        <td className="py-1 px-2">{inputListTahun.length == 0 && <input type="checkbox" className="w-3 h-3 text-blue-500 bg-gray-100 border-gray-300 rounded focus:ring-transparent" checked={selectedRows.tabelTahunRows.some((row) => row.id === item.id)} onChange={() => handleCheckboxChange(item, setSelectedRows, 'tahun')} />}</td>
